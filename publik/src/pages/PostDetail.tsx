...
<section className="mb-6">
  <h2 className="text-lg font-extrabold mb-3">Berita Terkait</h2> {/* 🆕 font-extrabold */}
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
    {related.map((p) => (
      <Link key={p.id} to={`/post/${p.slug}`} className="block border rounded-lg overflow-hidden hover:shadow-md transition">
        ...
        <div className="p-2 text-sm font-semibold">{p.title}</div> {/* 🆕 font-semibold */}
      </Link>
    ))}
  </div>
</section>
